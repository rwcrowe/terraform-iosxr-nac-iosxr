        ...
        for vrf in try(bgp_process.vrfs, []) : [
          for neighbor in try(vrf.neighbors, []) : [
            for af in try(neighbor.address_family, []) : {
              ...
              site_of_origin = try(af.site_of_origin, local.defaults.iosxr.devices.configuration.routing.bgp.vrfs.neighbors.address_family.site_of_origin, null) != null ? provider::utils::parse_bgp_rd_rt(
