    ...
    for device in local.devices : [
      for bgp_process in try(local.device_config[device.name].routing.bgp, []) : [
        for session_group in try(bgp_process.session_groups, []) : {
          ...
          local_as_no_prepend                              = try(session_group.local_as_mode, local.defaults.iosxr.devices.configuration.routing.bgp.session_groups.local_as_mode, null) == "no-prepend" ? true : null
          local_as_no_prepend_replace_as                   = try(session_group.local_as_mode, local.defaults.iosxr.devices.configuration.routing.bgp.session_groups.local_as_mode, null) == "no-prepend-replace-as" ? true : null
          local_as_no_prepend_replace_as_dual_as           = try(session_group.local_as_mode, local.defaults.iosxr.devices.configuration.routing.bgp.session_groups.local_as_mode, null) == "no-prepend-replace-as-dual-as" ? true : null
          ...
          dscp = try(lookup(local.dscp_map,
            tostring(try(session_group.dscp, local.defaults.iosxr.devices.configuration.routing.bgp.session_groups.dscp)),
            tostring(try(session_group.dscp, local.defaults.iosxr.devices.configuration.routing.bgp.session_groups.dscp))
            ), null
          )
          precedence = try(lookup(local.precedence_map,
            tostring(try(session_group.precedence, local.defaults.iosxr.devices.configuration.routing.bgp.session_groups.precedence)),
            tostring(try(session_group.precedence, local.defaults.iosxr.devices.configuration.routing.bgp.session_groups.precedence))
            ), null
          )
