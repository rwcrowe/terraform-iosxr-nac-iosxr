      ...
      for bgp_process in try(local.device_config[device.name].routing.bgp, []) : [
        for neighbor in try(bgp_process.neighbors, []) : [
          for af in try(neighbor.address_family, []) : {
            ...
            site_of_origin = try(af.site_of_origin, local.defaults.iosxr.devices.configuration.routing.bgp.neighbors.address_family.site_of_origin, null) != null ? provider::utils::normalize_bgp_rd(
              try(af.site_of_origin, local.defaults.iosxr.devices.configuration.routing.bgp.neighbors.address_family.site_of_origin)
            ) : null
