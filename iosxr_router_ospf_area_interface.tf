      ...
      for ospf_process in try(local.device_config[device.name].routing.ospf_processes, []) : [
        for area in try(ospf_process.areas, []) : [
          for interface in try(area.interfaces, []) : {
            ...
            authentication_message_digest                                   = try(interface.authentication.type, local.defaults.iosxr.devices.configuration.routing.ospf_processes.areas.interfaces.authentication.type, null) == "message_digest" ? true : null
